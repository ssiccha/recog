...
function(ri, c, eps, N)
    ...
    while i <= S and nrPrebolsteringElms <= R do
        ...
        cr := c ^ r;
        cr2 := cr ^ r;
        if not arecommuting(ri)(cr, c)
                and not isequal(ri)(cr2, c)
                and not isequal(ri)(cr2, c ^ 2)
                and arecommuting(ri)(cr2, c)
        then
...
function(ri, g, c, r, s, k, k0)
    ...
    for j in [1 .. k0 - 1] do
        # invariant: x = c ^ (r ^ j)
        x := x ^ r;
        if arecommuting(ri)(x, gTilde * c ^ 2) then
...
function(ri, c, x, N)
    ...
    if not IsElmOfPrimeOrder(ri, d * c, 2) then
        # Case 1, alpha > beta
        if arecommuting(ri)(dx, d ^ c) then
            e := dx ^ c;
        # Case 2, alpha < beta
        else
            e := (dx ^ (c ^ 2)) ^ 2;
    ...
    # w not in v ^ <x>
        ...
        # Case 3, alpha > beta
        if not arecommuting(ri)(dx, d ^ c) then
            e := dx ^ (c ^ 2);
        # Case 4, alpha < beta
        else
            e := (dx ^ c) ^ 2;
